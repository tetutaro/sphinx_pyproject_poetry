...
.PHONY: stubs
stubs:
	stubgen -o . -p sphinx_pyproject_poetry

.PHONY: build-package
build-package:
	$(eval VERSION := $(shell poetry version -s))
	poetry build
	@tar zxf dist/sphinx_pyproject_poetry-$(VERSION).tar.gz -C ./dist
	@cp dist/sphinx_pyproject_poetry-$(VERSION)/setup.py setup.py
	@black setup.py
	@rm -rf dist
...
.PHONY: uninstall
uninstall:
	pip uninstall -y sphinx-pyproject-poetry
...
ifdef VERSION
	...
	git add pyproject.toml sphinx_pyproject_poetry/__init__.py
